...
Original file is located at
    https://colab.research.google.com/drive/1yGkq0ILshSTYqxqkfesQsxlsO_lnmD_W
"""
...
"""Using feature importance we realized that 99% of the importance was found within these five variables(value_eur, release_clause_eur, age, potential, movement_reactions) and the rest of the features are infinitesimal. Hence we stuck with just these five features."""
...
model_gs = GridSearchCV(rf, param_grid=PARAMETERS, cv=cv, scoring="neg_mean_squared_error", n_jobs = -1)

model_gs.fit(Xtrain, Ytrain)

bp = model_gs.best_params_

rf = RandomForestRegressor(**bp)
...
one_player = rf.predict(sc.transform([[80000, 10000, 33, 94.0, 90.0]]))
one_player
...
xgbmodel_gs = GridSearchCV(xgb, param_grid=PARAMETERS, cv=cv, scoring="neg_mean_squared_error", n_jobs = -1)
...
gbmodel_gs = GridSearchCV(gbr, param_grid=PARAMETERS, cv=cv, scoring="neg_mean_squared_error", n_jobs = -1)
...
bp = gbmodel_gs.best_params_
gb = GradientBoostingRegressor(**bp)
...
# Specify an absolute path where you want to save the file
file_path = '/content/drive/My Drive/scaler1.pkl'
